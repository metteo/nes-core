...
public class DelegatingRegister extends Register {
    ...
    private ByteRegister byteRegister = nullByteRegister;
    ...
    public void configureByteRegister(ByteRegister byteRegister) {
        reset();
    ...
    class MemoryDelegate implements Delegate {
        ...
        @Override
        public @Unsigned short getAddress() {
            @Unsigned byte addressLo = memoryBus.specifyAnd(address).readByte();
            @Unsigned byte  addressHi = memoryBus.specifyAnd(ushort(uint(address) + 1)).readByte();

            return ushort((uint(addressHi) << 8) | uint(addressLo));
        }
    }
}
