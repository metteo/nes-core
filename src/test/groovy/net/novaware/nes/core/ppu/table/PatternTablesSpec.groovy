...
class PatternTablesSpec extends Specification {
    ...
    def "should construct single pattern address"() {
        when:
        def address = PatternTables.getAddress(SINGLE, tab, row, col, plane, sliver)

        then:
        address == outAddress

        where:
        tab | row    | col    | plane | sliver || outAddress
        //  |   rrrr |   cccc |       |        ||       rrrr_cccc
        //  |        |        |       |        ||   0_T_CCCC_CCCC_P_SSS
    ...
    def "should construct double pattern address"() {
        when:
        def address = PatternTables.getAddress(DOUBLE, tab, row, col, plane, sliver)

        then:
        address == outAddress

        where:
        tab | row    | col   | half | plane | sliver   || outAddress             | comment
        //  |   rrrr |   ccc |      |       |   h_sss  ||       rrrr_ccc_h_._sss |
        //T |   CCCC |   CCC |      |     P |   S_SSS  ||   0_T_CCCC_CCC_S_P_SSS |
        0b0 | 0b0000 | 0b000 | _    |   0b0 | 0b0_000  || 0b0_0_0000_000_0_0_000 | "zeroes"
        0b0 | 0b0000 | 0b000 | _    |   0b0 | 0b0_111  || 0b0_0_0000_000_0_0_111 | "sliver 0-7"
        0b0 | 0b0000 | 0b000 | _    |   0b0 | 0b1_000  || 0b0_0_0000_000_1_0_000 | "sliver 8-15"
        ...
        0b0 | 0b0000 | 0b000 | _    |   0b0 | 0b1_111  || 0b0_0_0000_000_1_0_111 | "sliver 0-15"
